      ...
      {open && (
        <div className="border-t border-gray-100 p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5 pt-4 border-t border-gray-100">
            {[
              { label: 'Scoring model', value: 'Claude Sonnet' },
              { label: 'Outreach drafts', value: 'Claude Sonnet' },
              { label: 'Data source', value: 'Supabase' },
              { label: 'Scraper', value: 'Apify' },
            ].map(({ label, value }) => (
              <div key={label} className="bg-gray-50 rounded-lg px-3 py-2.5 text-center">
                <div className="text-xs text-gray-400">{label}</div>
                <div className="font-medium text-gray-700 mt-0.5 text-sm">{value}</div>
              </div>
            ))}
          </div>
        </div>
